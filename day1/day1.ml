...
let make_lists lines =
  let rec aux lines list1 list2 =
    ...
    | [] -> list1, list2
    | head :: tail ->
      let split = String.split ~on:' ' head |> List.filter ~f:(String.( <> ) "") in
      aux
        tail
...
let calculate_distance lists =
  let rec aux lists sum =
    match lists with
    | [], _ | _, [] -> sum
    | head1 :: tail1, head2 :: tail2 -> aux (tail1, tail2) (Int.abs (head1 - head2) + sum)
  in
  aux lists 0
;;
...
let () =
  let lists = In_channel.read_lines "day1/input.txt" |> make_lists |> sort_lists in
